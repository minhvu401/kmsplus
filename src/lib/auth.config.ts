        ...
        try {
          ...
          if (result && result.length > 0) {
            ...
            if (
              ...
              existingUser.password_hash !== "google_oauth"
            ) {
              // Account exists with credentials - allow linking if user explicitly using Google
              ;`Linking Google OAuth to existing credentials account: ${existingUser.email}`
            }
            ...
            try {
              ...
              if (insertError?.code === "23505") {
                console.warn(
                  `Duplicate email during Google OAuth signup: ${user.email}`
                )
                // Try to fetch again
