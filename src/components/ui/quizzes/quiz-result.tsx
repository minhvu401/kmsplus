        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            ...
            <Row gutter={[24, 24]}>
                ...
                <Col xs={24} md={8}>
                    <Card bordered={false} style={{ height: '100%', borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
                         <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', height: '100%', justifyContent: 'center' }}>
                            <TrophyOutlined style={{ fontSize: 36, color: '#52c41a', marginBottom: 16 }} />
                            <Text type="secondary" style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: '1px' }}>Performance</Text>
                            <Space align="baseline" style={{ marginTop: 4 }}>
                                <Title level={3} style={{ margin: 0, color: '#52c41a' }}>{correctCount}</Title>
                ...
                <Col xs={24} md={8}>
                    <Card bordered={false} style={{ height: '100%', borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', height: '100%', justifyContent: 'center' }}>
                            <ClockCircleOutlined style={{ fontSize: 36, color: '#1677ff', marginBottom: 16 }} />
                            <Text type="secondary" style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: '1px' }}>Time Taken</Text>
                            <Title level={3} style={{ margin: '4px 0 0' }}>{formatTime(result.time_spent_seconds)}</Title>
                        </div>
                    </Card>
                </Col>
            </Row>
...
function QuestionCard({ question, index, total }: { question: QuestionResult, index: number, total: number }) {
        ...
        >
            <div style={{ padding: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                    ...
                    <Tag icon={isCorrect ? <CheckCircleFilled /> : <CloseCircleFilled />} color={isCorrect ? "success" : "error"}>
                ...
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                    {optionsList.map(([key, text]) => {
                        ...
                        let icon = <div style={{ width: 20, height: 20, borderRadius: '50%', border: '2px solid #d9d9d9', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}><span style={{ fontSize: 10, color: '#8c8c8c' }}>{key}</span></div>;
                        let textColor = 'inherit';

                        if (isRightAnswer) {
                            borderColor = '#b7eb8f'; // Light green border
                            bgColor = '#f6ffed'; // Light green bg
                            icon = <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#52c41a', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}><CheckOutlined style={{ color: 'white', fontSize: 12 }} /></div>;
                        } else if (isSelected && !isRightAnswer) {
                            borderColor = '#ffa39e'; // Light red border
                            bgColor = '#fff1f0'; // Light red bg
                            icon = <div style={{ width: 20, height: 20, borderRadius: '50%', background: '#ff4d4f', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}><CloseOutlined style={{ color: 'white', fontSize: 12 }} /></div>;
                        }
                        ...
                        return (
                            ...
                            >
                                ...
                                {isSelected && !isRightAnswer && <Text type="danger" style={{ fontSize: 12, fontWeight: 600 }}>Your Answer</Text>}
                                {isSelected && isRightAnswer && <Text type="success" style={{ fontSize: 12, fontWeight: 600 }}>Your Answer</Text>}
                            </div>
                        );
                    })}
                </Space>
            ...
            {(question.explanation || !isCorrect) && (
                <div style={{ 
                    ...
                    <Space size={8} style={{ marginBottom: 8 }}>
                         ...
                         <Text strong style={{ color: '#1677ff' }}>Instructor Feedback</Text>
                    </Space>
                    <Paragraph style={{ margin: 0, color: '#595959' }}>
                        {question.explanation ? question.explanation : (
                             <>The correct answer is <strong>{correctAnswers.join(', ')}</strong>.</>
                        )}
                    </Paragraph>
                </div>
            )}
